...
#removing linechanges
text_nolines = text_lower.replace("\n", "")

#removing punctuation, can't remove "-" for topics could be related to "1970-luku" or similar
#also numers can't be removed
def remove_punc2(text_nolines):
    punctuation = '!@#$%^&*()_+={}[]:;"”\'|<>,.?/~`■'
    clean_text = ""
    for character in text_nolines:
        if character not in punctuation:
           clean_text += character
    return clean_text

text_nopunc = remove_punc2(text_nolines)
#print(text_nopunc)

#removing stopwords
def removeStopwords(text_nopunc, stopwords):
   no_stopwords = []
   for w in text_nopunc.split(" "):
        if w not in stopwords:
            no_stopwords.append(w)    
   return no_stopwords

text_no_stopwords = removeStopwords(text_nopunc,stopwords)
...
#writing the results to a single file for MALLET
outfile = open("cleantext.txt", mode="w", encoding="utf-8")
